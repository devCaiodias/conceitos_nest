...
describe('PessoasController', () => {
  ...
  it(`remove - deve usar o PessoaService com o argumento correto`, async () => {
    const argument = 1
    ...
    const result = await controller.remove(argument as any, argument2 as any)

    expect(pessoasServiceMock.remove).toHaveBeenCalledWith(+argument, argument2)
    expect(result).toEqual(expected)
  })
